...
/** 
* @author 作者:majingji
* @version 创建时间：2019年12月5日 下午2:07:11 
* 类功能说明 
*/
...
//			if (url != null && Pattern.matches(regex, url)) {
				...
				if (articleContentElement != null) {
					...
					//写入到文件中
					FileUtil.writeFile("D:\\1707EJsoup\\" + title + ".txt", content, "utf8");
	...
	public void testXS() throws IOException {
		...
		int count = 0;
		// 获取连接对象
		Connection connect = Jsoup.connect("https://www.biqiugex.com/book_563/");
		// 获取文档对象
		Document document = connect.get();
		// 获取当前文档的所有超链接
		Elements ahrefs = document.select("a[href]");
...
//			String regex = "https://news\\\\.163\\\\.com.*html$";
			...
			//https://news\\.163\\.com.*html$
			// 特殊要求  
			if (url != null && url.startsWith("/book_563") && url.endsWith("html")) {
//			if (url != null && Pattern.matches(regex, url)) {
				// 连接的文本内容
				String title = href.text();
